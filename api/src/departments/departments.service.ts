...
export class DepartmentsService {
    ...
    async create(departmentDto: DepartmentDto) {
        await this.validateData(departmentDto.organization_id, departmentDto.parent_id);

        const res = await this.conn.query(`
            INSERT INTO departments (name, comment, organization_id, parent_id)
            VALUES ($1, $2, $3, $4)
            RETURNING *`,
            [departmentDto.name, departmentDto.comment, departmentDto.organization_id, departmentDto.parent_id]
        );
    ...
    async update(id: number, departmentDto: DepartmentDto) {
        ...
        if (id === departmentDto.parent_id) {
            throw new BadRequestException('Отдел не может быть родителем самому себе');
        }
        ...
        const res = await this.conn.query(`
            ...
            RETURNING *`,
            [departmentDto.name, departmentDto.comment, departmentDto.organization_id, departmentDto.parent_id, id]
        );
    ...
    async validateData(organizationId: number, parentId?: number) {
        ...
        if (parentId) {
            ...
            if (parentDepartment.rows[0].organization_id != organizationId) {
                throw new BadRequestException('Родительский отдел и создаваемый/изменяемый отдел принадлежат разным организациям');
            }
        }
    }
}
