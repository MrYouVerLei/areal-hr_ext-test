...
async function changeSalaryHrOperation(employeeId) {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/hr-operations`, {
      ...
      body: JSON.stringify({
        ...
        operation_type: 'Изменение зарплаты',
...
async function loadSalaryData(employeeId) {
  try {
    ...
    if (employeeData.value.department_id) {
      salary.value = employeeData.value.salary;
    }
  } catch (err) {
    console.error(err);
  }
...
<template>
  <q-dialog ref="dialogRef" @hide="onDialogHide">
    <q-card style="width: 400px; max-width: 40vw">
      <div class="q-pa-md">
        <q-form @submit="changeSalaryHrOperation(props.id)">
          <q-card-section class="text-center">
            <h5 class="q-ma-none">Изменение зарплаты</h5>
          </q-card-section>
